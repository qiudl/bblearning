...
final class QuestionAnswerViewModel: BaseViewModel {
    ...
    func submitAnswer() {
        ...
        executeTask(
            ...
            onSuccess: { [weak self] record in
                // Convert PracticeRecord to AnswerFeedback for UI
                let feedback = AnswerFeedback(
                    isCorrect: record.isCorrect,
                    score: Double(record.score),
                    standardAnswer: record.question?.answer.content ?? "",
                    explanation: record.aiGrade?.feedback,
                    hints: record.aiGrade?.suggestions
                )
