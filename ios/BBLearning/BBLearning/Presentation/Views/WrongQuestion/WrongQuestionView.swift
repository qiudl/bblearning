    ...
    var body: some View {
        ...
        .fullScreenCover(isPresented: $viewModel.showRetryView) {
            if let question = viewModel.selectedQuestion {
                QuestionView(questions: [question.question])
            }
        }
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            HStack {
                ...
                Text(wrongQuestion.wrongTime.formatted(date: .abbreviated, time: .omitted))
                    .font(.caption)
                    .foregroundColor(.textSecondary)
            }
...
struct FilterSheet: View {
    ...
    var body: some View {
        NavigationView {
            List {
                Section("状态") {
                    ForEach([WrongQuestion.Status.pending, .retrying, .mastered], id: \.self) { status in
                        Button(action: {
...
extension WrongQuestion.Status {
