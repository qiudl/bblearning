...
final class GenerateQuestionsUseCase {
    ...
    func execute(knowledgePointIds: [Int], count: Int = 10, difficulty: Question.Difficulty? = nil) -> AnyPublisher<[Question], APIError> {
        ...
        guard !knowledgePointIds.isEmpty else {
            return Fail(error: APIError.parameterError("请选择至少一个知识点")).eraseToAnyPublisher()
        }

        guard count > 0 && count <= 50 else {
            return Fail(error: APIError.parameterError("题目数量应在1-50之间")).eraseToAnyPublisher()
        }
    ...
    func generateComprehensive(
        ...
        guard !knowledgePointIds.isEmpty else {
            return Fail(error: APIError.parameterError("请选择知识点")).eraseToAnyPublisher()
        }
...
enum DifficultyDistribution {
    ...
    func calculateCounts(total: Int) -> (easy: Int, medium: Int, hard: Int) {
        let (easyRatio, mediumRatio, hardRatio): (Double, Double, Double)

        switch self {
        case .easy:
            (easyRatio, mediumRatio, hardRatio) = (0.7, 0.25, 0.05)
        case .balanced:
            (easyRatio, mediumRatio, hardRatio) = (0.3, 0.5, 0.2)
        case .challenging:
            (easyRatio, mediumRatio, hardRatio) = (0.1, 0.4, 0.5)
        case .custom(let e, let m, let h):
            let sum = e + m + h
            (easyRatio, mediumRatio, hardRatio) = (e / sum, m / sum, h / sum)
        }
