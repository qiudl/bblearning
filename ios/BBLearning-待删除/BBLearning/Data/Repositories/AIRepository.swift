...
final class AIRepository: AIRepositoryProtocol {
    ...
    func chat(conversationId: Int?, message: String) -> AnyPublisher<AIMessage, APIError> {
        let endpoint = AIEndpoint.chat(message: message, conversationId: conversationId)
    ...
    func recognizeQuestion(imageData: Data) -> AnyPublisher<QuestionRecognitionResult, APIError> {
        _ = AIEndpoint.recognizeQuestion(imageData: imageData)
        // Need to implement upload with proper parameters
        return Fail(error: APIError.badRequest("Upload not implemented"))
            .eraseToAnyPublisher()
    }
    ...
    func getRecommendations(limit: Int) -> AnyPublisher<[RecommendedPractice], APIError> {
        let endpoint = AIEndpoint.recommendPractices(limit: limit)
        return apiClient.request(endpoint, type: [RecommendedPractice].self)
    }
